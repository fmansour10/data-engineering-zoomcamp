...
YEARS = [2020]
MONTHS = [f"{i:02d}" for i in range(1, 13)]  # All 12 months
DOWNLOAD_DIR = "."
...
if __name__ == "__main__":
    ...
    print(f"Total files to download: {len(download_params)}")
    ...
    print(f"\nSuccessfully downloaded {len(successful_downloads)} out of {len(download_params)} files")
    
    with ThreadPoolExecutor(max_workers=4) as executor:
        executor.map(upload_to_gcs, successful_downloads)
